...
| Group by project | On (default): tiles sharing a project sit next to each other on the wall, pinned tiles still leading. Off keeps whatever order dragging and splitting already put them in |
...
| Terminal minimized | Not on the settings screen: the panes taken off the grid with `Alt+M`, one session id or project path per `\|`. A minimized pane keeps running, moves into the tray under the strip, and takes no number of its own until it is brought back. Delete the line to restore everything |
