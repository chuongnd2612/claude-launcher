...
| Terminals (terminal tile focused) | every key goes to Claude — its own UI, prompts and pickers, `Esc` and `Tab` included · `Ctrl+T` new terminal · `Alt+\` / `Alt+-` split this pane in two, side by side or one above the other, and start a session in the new half · `Alt+,` / `Alt+.` / `Alt+/` move this pane into the tile before it, the tile after it, or a tile of its own · `Ctrl+W` close this terminal · `Alt+Z` zoom this pane · `Ctrl+F` find text, `Enter` searches back through the screen history, `Tab` searches the whole session · `Alt+S` select text · `Alt+1..9` jump to a pane · `Alt+←→↑↓` step between panes · `Ctrl+Shift+←→↑↓` move this pane · `Alt+Shift+←→↑↓` resize them · `Alt+Shift+0` even them up · `Shift+PgUp/PgDn` scroll Claude’s history · `F1` keys · `Ctrl+]` or a click off the tiles releases the keyboard · `Ctrl+]`, `Enter` or a click on a tile resumes typing. All six of those chords are rebindable |
| Terminals (the mouse) | click a tile to focus it and type into it · **triple-click a tile to close it** · drag a tile onto another to move it · drag a gutter or an interior divider to resize · the wheel scrolls whatever is under the pointer · clicking off the tiles hands the keyboard back |
