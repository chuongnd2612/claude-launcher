...
| Terminals (usage over the tiles) | `Alt+G` draws each pane's account usage over its top-right corner, and hides it again — a gauge, percentage and countdown for the `5h` and weekly windows, the same figures as the header band. Works from a focused terminal tile, and asks the band for a fresh reading as it opens |
