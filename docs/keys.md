...
| Terminals (the wall — no tile holding the keyboard) | `1..9` focus · `↑↓←→` / `Tab` move · `Enter` attach · `Ctrl+Shift+←→↑↓` move the focused pane along the wall · `Alt+Shift+←→↑↓` resize the panes · `Alt+Shift+0` even them up · `z` zoom · `Space` layout · `t` new terminal · `n` new session · `w` close a terminal, or hide a session that is not ours · `Esc` back · `q` quit · `v`/`s` split the focused pane in two, side by side / one above the other, and pick what starts in the new half (with terminal tiles **off** these split into Windows Terminal panes instead) · `Alt+\` and `Alt+-` do the same from anywhere on the wall · `Alt+,` / `Alt+.` move the focused pane into the tile before / after it and `Alt+/` moves it out into a tile of its own · `Alt+Shift+←→↑↓` resizes the split you are in before the wall around it |
...
| New pane picker (`Alt+\` / `Alt+-`, or `v`/`s` on the wall) | **project:** type to filter · `↑↓` pick · `Enter` next · `Esc` cancel the split · **how it starts:** `↑↓` or `n`/`c`/`r` · `←→` profile · `Enter` start it in this half · `Esc` back · **which one** (after Resume): `↑↓` pick · `Enter` resume here · `Esc` back |
| Terminals (terminal tile focused) | every key goes to Claude — its own UI, prompts and pickers, `Esc` and `Tab` included · `Ctrl+T` new terminal · `Alt+\` / `Alt+-` split this pane in two, side by side or one above the other, and start a session in the new half · `Alt+,` / `Alt+.` / `Alt+/` move this pane into the tile before it, the tile after it, or a tile of its own · `Alt+P` pin it, or unpin it · `Alt+G` usage over every tile · `Ctrl+W` close this terminal · `Alt+Z` zoom this pane · `Ctrl+F` find text, `Enter` searches back through the screen history, `Tab` searches the whole session · `Alt+S` select text · `Alt+1..9` jump to a pane · `Alt+←→↑↓` step between panes · `Ctrl+Shift+←→↑↓` move this pane · `Alt+Shift+←→↑↓` resize them · `Alt+Shift+0` even them up · `Shift+PgUp/PgDn` scroll Claude’s history · `F1` keys · `Ctrl+]` or a click off the tiles releases the keyboard · `Ctrl+]`, `Enter` or a click on a tile resumes typing. All six of those chords are rebindable |
| Terminals (usage over the tiles) | `Alt+G` draws each pane's account usage over its top-right corner, and hides it again — a gauge, percentage and countdown for the `5h` and weekly windows, the same figures as the header band. Opening it also asks Claude directly: a hidden session runs `/usage` for each profile with a real terminal on the wall, so the figure lands a few seconds later rather than waiting for someone to run it by hand. Works from a focused terminal tile |
| Terminals (pinning) | `Alt+P` pin the focused pane, or unpin it — a pinned tile takes a double violet border and a `★`, leads the wall, refuses to close until it is unpinned, and is resumed on its own the next time the launcher starts |
| Terminals (minimizing) | `Alt+M` takes the focused tile off the grid without closing it, or brings it back — a minimized tile moves into a small tray under the strip, listed by project, and the freed space goes to whatever is still on the wall. It keeps no number of its own while minimized, so click a tray entry to restore and focus it (triple-click closes it, same as any tile); landing on its pane any other way — `Alt+1..9`, a search hit — restores it too, since there is nowhere on the wall for the keyboard to go otherwise |
| Terminals (the mouse) | click a tile to focus it and type into it · **triple-click a tile to close it** (a pinned tile refuses) · drag a tile onto another to move it · drag a gutter or an interior divider to resize · **click a project's name in the strip to show only that project's tiles**, and click it again to show everything · the wheel scrolls whatever is under the pointer · clicking off the tiles hands the keyboard back |
